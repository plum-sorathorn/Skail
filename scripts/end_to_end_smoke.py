...
# Ensure repo root on sys.path so `autoconduck` is importable without pip install -e
try:
...
class _Target:
    ...
    def __init__(self, base: str | None, use_testclient: bool = False):
        ...
        if use_testclient:
            ...
            # For TestClient fallback, auto-enable plugins unless AUTOCONDUCK_HOME
            ...
            _explicit_home = _os_smoke.environ.get("AUTOCONDUCK_HOME")
            try:
...
def main():
    parser = argparse.ArgumentParser(description="AutoConduck end-to-end smoke")
