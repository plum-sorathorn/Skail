# Skail adaptive orchestration and release implementation guide

Status: **Ready for phased implementation; no phase or release gate is certified by this document.**
Prepared: 2026-09-05
Workspace: `C:\Users\plum\Documents\Works\Skail`
Branch: `skail`
...
Read the applicable `AGENTS.md`, accepted ADRs, [SPEC](../docs/skail/SPEC.md), [architecture](../docs/skail/ARCHITECTURE.md), and [feature contracts](../docs/skail/FEATURES.md) before their subsystem changes. Query Graphify before locating unfamiliar code.

The existing [remediation tracker](skail-v0.1.0-release-remediation.md) and [historical implementation plan](plan.md) remain historical records. Phase 00 establishes this guide as the active sequence and adds pointers without deleting history. Do not silently leave conflicting active instructions behind.
...
These assignments concern the agents **implementing Skail**, not hardcoded model choices inside Skail's router.
...
5. After two unsuccessful attempts at the same implementation defect without new evidence, stop guessing: produce a bounded diagnosis for Terra or the next Sol checkpoint. This is a development-efficiency rule, distinct from Skail's runtime attempt contract.
...
- Run `python -m ruff check src tests scripts evals benchmarks` and `python -m mypy src\skail` for Python changes. Extend focused type coverage to new typed evaluation helpers; do not hide errors by dropping paths or adding blanket ignores.
...
**Acceptance:** no conflicting active execution/release instructions; old completed work is preserved; numbered phases can proceed without another architecture permission question. Commit `docs(skail): define adaptive execution and release contracts` and check in.
...
bounded context packet. References stay opaque and require existing enabled Skail tools, whose
...
[the Phase 10 review record](skail-phase-10-graph-accounting-review.md).
...
Added task-specific managed worktrees from a verified immutable snapshot. Materialization verifies the source Git base and every copied digest, uses a Skail-owned task branch, rejects unsafe task/path values, and preserves any worktree with unintegrated changes using durable retention metadata. The final 11c wiring must give admitted isolated children their managed root and expose the selected/fallback workspace mode without weakening shared-workspace leases.
...
Capture actual effects only from the Skail-owned task worktree bound to its snapshot and attempt.
...
child subprocess cannot be confined by Skail's current structured-command policy. The shared
...
**Status:** complete. Managed capture now authenticates the exact Skail-owned worktree path,
...
**Status:** complete. **Scope:** adaptive plan nodes and dependencies, active execution mode, route reasons, evidence status, workspace integration changesets, actual/estimated/reserved/unknown costs, snapshotted/event-driven projection, and Textual pilot test coverage.

**Handoff (completed 2026-09-09):** Extended `TuiProjection` and `SessionSnapshot` with durable plan nodes, dependencies, workspace changesets, execution decisions, shadow routing recommendations/reasons, and unknown costs. Added `PlanView` widget displaying adaptive plan nodes and workspace integration changesets with distinct status styling (`ready`, `running`, `succeeded`, `failed`, `blocked`, `cancelled`, and `integrated`, `verified`, `pending`, `in_doubt`, `rejected`). Extended `RouteView` with shadow strategy labels (`[SHADOW]`), reasons, and evidence status. Extended `BudgetView` with unknown cost. Extended `AgentRail` to clearly distinguish planned, queued, executing, waiting approval, verified, integrated, complete, failed, blocked, and cancelled states. Subscribed event listener in `SkailApp.__init__` before runtime services run to capture early events before mount. Added `/plan` slash command and `ctrl+p` binding. Verified live and resumed views agree across snapshots and events. All 31 focused TUI tests, 302 unit tests, 190 integration tests, 123 contract/security/eval tests, and 633 full offline tests passed; Ruff, strict mypy (110 source files), and fake-provider smoke passed. `graphify update .` updated (245/245 files). Next: Phase 15 paired evaluation of the completed runtime.
...
[Phase 16 Economics and Isolation Review](../docs/skail/PHASE_16_REVIEW.md).
...
Protected/unrelated files preserved: pre-existing user edits in AGENTS.md, evals/runner.py, src/skail/runtime/run_controller.py, and tests/unit/test_eval_runner.py were not staged or modified by Phase 16. `.gitignore` and evals/results/run_1.json, run_1.md, run_2.json, and run_2.md were not modified or staged.
...
Behavior delivered: `scripts/package_check.py` builds exactly one wheel and sdist outside the checkout, inspects both for runtime-only contents, installs the primary wheel into a fresh temporary virtual environment from outside the source tree without resolving dependencies from the network, runs installed `skail --help`, `skail --version`, and fake-provider smoke, and emits source SHA, environment metadata, and SHA-256 artifact evidence. Release CI invokes this verifier on its Windows and Linux matrix and uploads the evidence outside the checkout.
...
External evidence location and source identity, if applicable: external raw evidence path is `C:\Users\plum\AppData\Local\Temp\skail-phase19-rendered-evidence.json`; regenerate it after the phase commit so its `source_commit` matches the candidate.
Protected/unrelated files preserved: `.gitignore` and `evals/results/run_1.json`, `run_1.md`, `run_2.json`, and `run_2.md` were not modified or staged.
...
limitation in docs/skail/DEPENDENCIES.md. PERFORMANCE.md now defines the operation measured by each
...
`C:\Users\plum\AppData\Local\Temp\skail-phase19-final-42a5d01dfee448b397e4d47e976279c5.evidence.json`,
and isolated package evidence is retained at
`C:\Users\plum\AppData\Local\Temp\skail-package-final-79ed90465caf42f8bcf0660fe330ca04.evidence.json`.
...
**Acceptance:** each active capability claim has tests/evidence or an explicit experimental/unverified label; repository URLs unchanged; docs distinguish engineering release from Q1. Commit `docs(skail): align adaptive runtime documentation and roadmap`.
...
2. Inventory every tracked prior-product identity reference before changing it, including the
distribution/package metadata, console entry point, Python import/package names, CLI/TUI strings,
configuration/session paths, documentation, examples, tests, fixtures, scripts, CI/release
metadata, repository/community files, and release-plan/handoff text. Replace each with the selected
Skail form or `skail` identifier as appropriate. Do not leave compatibility aliases, redirects,
deprecated commands, or dual branding.
...
- Verify no runtime, packaging, tests, scripts, or documentation commands depend on the archive. Identify exact deletion targets within `legacy/skail/` and review the manifest.
...
in C:\Users\plum\Documents\Works\Skail on branch skail.
...
Behavior delivered: ADR 0006 accepts adaptive typed plans, task compatibility, isolated-writer and schema ownership contracts, model stickiness, outcome qualification, and separate engineering/economic release boundaries. Active and historical trackers are reconciled. The active guide and its copyable phase-start prompt name the canonical `Skail` workspace; intentional Skail legacy and migration references remain unchanged.
Acceptance evidence and commands: `rtk pytest tests\unit\test_documentation_contracts.py -q`; `rtk pytest`; `python -m ruff check src tests scripts evals benchmarks`; `python -m mypy src\skail`; targeted content/link review; `graphify update .`; `rtk git diff --check`; complete diff review
...
Acceptance evidence and commands: `rtk pytest tests\unit\test_requirements.py tests\unit\test_selector.py tests\integration\test_assignment.py tests\integration\test_lead.py -q`; `rtk pytest`; `python -m ruff check src tests scripts evals benchmarks`; `python -m mypy src\skail`; `graphify update .`; `rtk git diff --check`; complete diff review
...
Acceptance evidence and commands: `rtk pytest tests\unit\test_estimates.py tests\unit\test_selector.py tests\integration\test_assignment.py tests\integration\test_lead.py -q`; `rtk pytest`; `python -m ruff check src tests scripts evals benchmarks`; `python -m mypy src\skail`; `graphify update .`; `rtk git diff --check`; complete diff review
...
Acceptance evidence and commands: `rtk pytest tests\unit\test_event_bus.py tests\unit\test_tui_projection.py tests\integration\test_lead.py tests\integration\test_assignment.py -q`; `rtk pytest`; `python -m ruff check src tests scripts evals benchmarks`; `python -m mypy src\skail`; `python scripts\smoke.py --fake-provider`; `graphify update .`; `rtk git diff --check`; complete diff review
...
Acceptance evidence and commands: `rtk pytest tests\integration\test_lead.py -k terminal_failure_event_observes_committed_failed_run -q`; affected event/controller suite; `python -m ruff check src tests scripts evals benchmarks`; `python -m mypy src\skail`; `graphify update .`; `rtk git diff --check`; complete diff review
...
Acceptance evidence and commands: `rtk pytest tests\contract\test_execution_decisions.py tests\integration\test_delegation_controls.py tests\integration\test_lead.py tests\integration\test_headless_core.py tests\integration\test_tui_commands.py tests\integration\test_task_graph.py tests\security\test_crash_recovery_and_reservations.py -q` (51 passed); `python -m mypy src\skail` (passed); `python -m ruff check src tests scripts evals benchmarks` (passed); `python scripts\smoke.py --fake-provider` (passed); `graphify update .`; `rtk git diff --check`; complete diff review.
...
Acceptance evidence and commands: `rtk pytest tests\unit\test_changesets.py tests\unit\test_journal.py tests\unit\test_workspaces.py tests\integration\test_delegation_controls.py tests\integration\test_write_leases.py tests\integration\test_filesystem_boundary.py -q`; `python -m ruff check src tests scripts evals benchmarks`; `python -m mypy src\skail`; `python scripts\smoke.py --fake-provider`; `rtk git diff --check`; `graphify update .`; complete diff review.
...
Phase: 20 — Documentation and separate feature-parity roadmap
Status: complete
Implementation model: available Codex model (assigned model treated as a recommendation)
Commit(s): recorded by the phase commit that follows this handoff update
Behavior delivered: Active product documentation now distinguishes normative contracts from tested or named evidence; the root README describes the actual CLI, adaptive/direct execution, worktree fallback, approval/steering boundaries, retry and concurrency limits, shadow economics, and release authority. The feature matrix records demonstrated scope and gaps for streaming, sessions/resume, approvals, steering, skills/memory, provider compatibility, inspection, and context management. A separate roadmap contains only editor integration, external interoperability, richer background-agent interaction, and multimodal discovery work.
Acceptance evidence and commands: `rtk pytest tests\unit\test_documentation_contracts.py -q`; CLI contract tests; offline smoke; static checks; full offline suite; Graphify update; Markdown link/content review; `rtk git diff --check`; complete staged-diff review.
Test results and documented skips: deterministic documentation and CLI checks cover active claims without a provider call. Live-provider quality/savings, exact-candidate Linux evidence, paid evaluation, publication, pushing, tagging, remote changes, and legacy deletion remain out of scope.
Review findings closed/open: stale root README saving/offline/provider-command claims, obsolete evaluation-phase wording, and unsupported absolute security/performance language were qualified or removed. No Phase 20 implementation finding remains open.
External evidence location and source identity, if applicable: none required for this documentation phase; candidate-specific release evidence is regenerated by later phases.
Protected/unrelated files preserved: `.gitignore` and `evals/results/run_1.json`, `run_1.md`, `run_2.json`, and `run_2.md` are excluded from phase staging.
Unproven claims or missing evidence: synthetic and fake-provider checks do not qualify live provider quality or economics; exact-candidate Windows/Linux evidence remains Phase 22/24 work.
Next phase and its dependencies: Phase 20a — Rename the product to Skail; depends on Phase 20.
```
...
Implementation model: available Codex model (assigned model treated as a recommendation)
Commit(s): recorded by the phase commit that follows this handoff update
Behavior delivered: The canonical full product name is Skail. The Python import and console command are `skail`, the distribution is `skail-harness`, runtime/configuration state uses `.skail`, and source, tests, fixtures, scripts, CI, release metadata, active documentation, and historical product material use the same identity. No compatibility command, import alias, redirect, or dual branding was retained.
Acceptance evidence and commands: focused identity/packaging regression tests; focused CLI/TUI/configuration/session/recovery tests; clean editable install; isolated wheel/sdist build and install; fake-provider smoke; full offline suite; Ruff; strict mypy; documentation content tests; whole-repository content/path scans; Graphify update; `rtk git diff --check`; complete staged-diff review.
Test results and documented skips: deterministic local verification passed. Live providers, paid evaluation, Linux CI evidence, pushing, publishing, tagging, remote rename, and external repository rename were not performed.
Review findings closed/open: corrected an inverted renamed CLI assertion, made mocked Git evidence robust to absent stdout, restored the former-package import boundary after a mechanical replacement, and aligned distribution metadata with the `skail-harness` project contract. No Phase 20a implementation finding remains open.
External evidence location and source identity, if applicable: the owner completed the external rename after Phase 20a. Final repository URL: `https://github.com/plum-sorathorn/Skail`. Final workspace: `C:\Users\plum\Documents\Works\Skail`. The root README, its documentation contract, and the local `origin` were reconciled in the bounded post-rename commit; no external evidence is required for this local documentation update.
Protected/unrelated files preserved: `.gitignore` and `evals/results/run_1.json`, `run_1.md`, `run_2.json`, and `run_2.md` were not modified or staged. Their protected historical contents are excluded from the editable-product scan by direct instruction.
...
Status: complete
Implementation model: available Codex model (assigned Terra treated as a recommendation)
Commit(s): candidate `b8ca9850eda399315cd3aad7693ddafadf3a6059`; readiness record is the phase commit following this handoff
Behavior delivered: Release verification now confines CLI smoke, full-suite runtime homes, paired evaluation state, and CLI runtime benchmarks outside the candidate checkout. The release-only pytest cleanliness contract attributes any test that creates checkout-root runtime state. CLI contract/E2E subprocesses and the benchmark aggregation fixture are self-contained, while Windows fixture bytes retain stable LF-based provenance.
Acceptance evidence and commands: clean detached candidate `C:\Users\plum\AppData\Local\Temp\skail-phase22-final-b8ca985\candidate`; `python -u scripts\release_check.py` passed all eight gates (clean input, Ruff over source/tests/scripts/evals/benchmarks, strict mypy, complete offline suite with release-root cleanliness assertions, documentation, fresh wheel inspection, fake-provider smoke, measured benchmark thresholds, and fresh complete paired evaluation); `python scripts\package_check.py --evidence C:\Users\plum\AppData\Local\Temp\skail-phase22-final-b8ca985\evidence\package-check.json` passed fresh wheel/sdist build, inspection, isolated no-dependency installation, installed help/version, and installed fake-provider smoke; `git status --porcelain=v1 --untracked-files=all` was empty after both verifiers.
Test results and documented skips: focused release-check tests passed (19); focused benchmark tests passed (7); the complete offline suite passed inside the integrated release verifier. Existing platform-capability skips remain represented by pytest and were not bypassed. Live providers, paid evaluation, Linux CI, pushing, publishing, tagging, remote changes, OAuth integration, and legacy cleanup were not run.
Review findings closed/open: Candidate contamination from CLI tests, developer smoke, full-suite runtime homes, paired evaluation, benchmark aggregation, and the real CLI benchmark is closed with executable regression coverage. No local Phase 22 defect remains open. Exact-commit Linux evidence is external and remains required by Phase 24; no Linux success or tag eligibility is claimed here.
External evidence location and source identity, if applicable: source `b8ca9850eda399315cd3aad7693ddafadf3a6059`; Windows `Windows-11-10.0.26200-SP0`; Python `3.14.6`; evidence directory `C:\Users\plum\AppData\Local\Temp\skail-phase22-final-b8ca985\evidence`; release stdout SHA-256 `6e0999e2c6ba5427eed4fa87bd542141dc8271b2bef10e60d4716132981eda2c`; empty release stderr SHA-256 `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`; package evidence SHA-256 `0c030d3ab880972e1237f9764fcaa38a001e92c814180d0cd16c4389158e87c5`; wheel SHA-256 `b2af04e7dc952fb49dd9aef5536370f9c22db43240139c74137a110123363ba5`; sdist SHA-256 `dcad33036d167847ce30e7faaceaa0b584a1d7d6dfa1c29fc7edc241e8885c8e`.
