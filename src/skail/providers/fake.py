...
class DeterministicFakeChatModel(BaseChatModel):
    ...
    response_text: str = "Task completed successfully by Skail fake provider."
        ...
        **kwargs: Any,
        ...
        for m in reversed(messages):
            if m.type == "human" and m.content:
                ...
                break
        content = self.response_text or f"Skail executed: {last_content}"
        generation = ChatGeneration(message=AIMessage(content=content))
        return ChatResult(generations=[generation])

    @property
    def _llm_type(self) -> str:
        return "skail-deterministic-fake"
...
class FakeProviderAdapter:
    ...
    def normalize_usage(self, response: object) -> NormalizedUsage | None:
        ...
        if isinstance(resp_meta, dict) and "skail_cost_usd" in resp_meta:
            raw_cost = Decimal(str(resp_meta["skail_cost_usd"]))
