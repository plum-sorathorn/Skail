...
from rudder.domain.changesets import ChangeSet, ChangeSetStatus, ContentImage
from rudder.runtime.workspace_capture import ImmutableArtifactStore, WorkspaceCaptureError
...
class ChangeSetIntegrator:
    ...
    def _write_image(self, target: Path, image: ContentImage) -> None:
        ...
        temporary = target.with_name(f".{target.name}.rudder-{uuid4().hex}")
        try:
