...
def load_config(
    *,
    ...
    layers: list[tuple[str, dict[str, Any]]] = [("default", RudderConfig().model_dump())]
    ...
    try:
        config = RudderConfig.model_validate(merged)
...
_TOP_LEVEL_FIELDS = frozenset(RudderConfig.model_fields)
_SECTION_FIELDS = {
    name: frozenset(field.annotation.model_fields)  # type: ignore[union-attr]
    for name, field in RudderConfig.model_fields.items()
