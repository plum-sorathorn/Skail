...
class OpenAICompatibleChatModel(BaseChatModel):
    ...
    def _llm_type(self) -> str:
        return f"rudder-{self._adapter.name}-openai-compatible"
        ...
        *,
        ...
        if response_schema is not None:
            ...
            body["response_format"] = {
                "type": "json_schema",
                "json_schema": {"name": "rudder_response", "schema": schema_value, "strict": True},
            }
        ...
        return AIMessage(
            ...
            response_metadata=(
                {"rudder_cost_usd": str(usage["cost"])} if "cost" in usage else {}
            ),
        )
...
class OpenAICompatibleAdapter:
    ...
    def normalize_usage(self, response: object) -> NormalizedUsage | None:
        ...
        if response.usage_metadata is None:
            return None
        raw_cost = response.response_metadata.get("rudder_cost_usd")
