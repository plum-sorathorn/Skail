...
AutoConduck imports OpenRouter Models and Benchmarks data only through an explicit maintenance operation. The imported data is atomically stored at `~/.autoconduck/run/benchmarks.json`, validated, and loaded at configuration/startup time. A routed request reads only the process-memory registry; it never calls OpenRouter or reads the snapshot file.
