...
TOOLS_MANIFEST = [
    {
        ...
        "description": (
            ...
            "(not semantic embedding). Run autoconduck_index first."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            "Index a directory into the AutoConduck knowledge base. "
            "Call before autoconduck_search on a new project."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
...
def _get_vector_store() -> Any:
    ...
    try:
        ...
        rag_db_path = (
            getattr(selection, "rag_db_path", "~/.autoconduck/rag_db")
            if selection is not None
            else "~/.autoconduck/rag_db"
        )
...
async def _mcp_tools_call_handler(request: Request) -> JSONResponse:
    """Handle MCP tool invocation for autoconduck_search and autoconduck_index."""
    global _indexed_at
    try:
        try:
        ...
        if tool_name == "autoconduck_search":
        ...
        elif tool_name == "autoconduck_index":
