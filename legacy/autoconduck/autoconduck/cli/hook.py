...
Invoked as `autoconduck hook claude <EventName>` from Claude Code hooks.
Design contract:
...
def _spool_path() -> Path:
    try:
        ...
        # fallback: ~/.autoconduck/run/plugin_spool.jsonl
        return Path.home() / ".autoconduck" / "run" / "plugin_spool.jsonl"
...
def cmd_hook(args) -> int:  # type: ignore[no-untyped-def]
    """Handle `autoconduck hook <harness> <event>`.
...
def register_hook_parser(subparsers) -> None:  # type: ignore[no-untyped-def]
    """Register `autoconduck hook` subcommand on given subparsers. Never raises."""
    try:
