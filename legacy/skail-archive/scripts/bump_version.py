#!/usr/bin/env python3
"""Single-source version bumper for Skail.

Updates the authoritative version in pyproject.toml (or skail/__init__.py)
...
INIT_FILE = ROOT / "skail" / "__init__.py"
...
def update_init(new_version: str) -> None:
    ...
    print(f"  [OK] Updated skail/__init__.py -> {new_version}")


def update_npm_packages(new_version: str) -> None:
    # Main package.json
    main_pkg = NPM_DIR / "skail" / "package.json"
    if main_pkg.is_file():
    ...
    for platform in PLATFORMS:
        pkg_file = NPM_DIR / f"skail-{platform}" / "package.json"
        if pkg_file.is_file():
...
def update_docs(new_version: str) -> None:
    ...
    if agents_file.is_file():
        ...
        updated = re.sub(r'Project:\s*\*\*Skail\*\*\s*\(`[^`]+`\s*in\s*`pyproject\.toml`\)', f'Project: **Skail** (`{new_version}` in `pyproject.toml`)', content)
    ...
    if readme_file.is_file():
        ...
        updated = re.sub(r'# Skail\s+\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?', f'# Skail {new_version}', content)
        updated = re.sub(r'\*\*Skail\s+\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?\*\*', f'**Skail {new_version}**', updated)
    ...
    if project_file.is_file():
        ...
        updated = re.sub(r'# Project:\s*Skail\s+\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?', f'# Project: Skail {new_version}', content)
        updated = re.sub(r'Skail\s+\d+\.\d+\.\d+(?:-[a-zA-Z0-9.]+)?\s+transforms', f'Skail {new_version} transforms', updated)
        project_file.write_text(updated, encoding="utf-8")
        print(f"  [OK] Updated PROJECT.md -> {new_version}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Synchronize version across all Skail files")
    ...
    print(f"Bumping Skail version: {current} -> {new_version}")
