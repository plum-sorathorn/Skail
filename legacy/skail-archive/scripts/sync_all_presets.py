...
def fetch_upstream_litellm_costs() -> dict[str, dict[str, Any]]:
    ...
    try:
        req = urllib.request.Request(
            UPSTREAM_LITELLM_URL, headers={"User-Agent": "skail/1.0"}
        )
        ...
        from skail.model_presets import _ingest_litellm_costs
...
def fetch_devpass_models(costs: dict[str, dict]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        from skail.presets.presets_fallback import FALLBACK_PRESETS
...
def fetch_llmgateway_models(costs: dict[str, dict]) -> list[dict[str, Any]]:
    ...
    try:
        req = urllib.request.Request(
            LLMGATEWAY_MODELS_URL, headers={"User-Agent": "skail/1.0"}
        )
        with urllib.request.urlopen(req, timeout=5) as resp:
            data = json.loads(resp.read().decode("utf-8"))
        entries = []
        from skail.model_presets import normalize_model_id_for_provider
...
def fetch_openrouter_models() -> list[dict[str, Any]]:
    ...
    try:
        req = urllib.request.Request(
            OPENROUTER_MODELS_URL, headers={"User-Agent": "skail/1.0"}
        )
        with urllib.request.urlopen(req, timeout=5) as resp:
            data = json.loads(resp.read().decode("utf-8"))
        entries = []
...
def build_provider_presets(
    ...
    from skail.presets.presets_ingest import clean_model_id
...
def merge_with_existing(new_presets: dict[str, list[dict[str, Any]]]) -> dict[str, list[dict[str, Any]]]:
    ...
    try:
        from skail.presets.presets_data import PRESETS as existing_presets
    except Exception:
        existing_presets = {}
...
def write_fallback_presets(presets: dict[str, list[dict[str, Any]]]) -> Path:
    ...
    target = ROOT / "skail" / "presets" / "presets_fallback.py"
...
def write_presets_data(presets: dict[str, list[dict[str, Any]]]) -> Path:
    """Atomically write formatted presets into skail/presets/presets_data.py."""
    import os
    import tempfile

    target = ROOT / "skail" / "presets" / "presets_data.py"
    target.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Write updated presets directly to skail/presets/presets_data.py (default: True)",
    )
    parser.add_argument(
        "--no-write",
        "--dry-run",
