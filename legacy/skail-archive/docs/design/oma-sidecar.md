...
Skail operates as a zero-overhead, per-turn Capability Floor Routing selector for everyday coding turns. However, high-complexity tasks (such as full architectural refactorings, multi-module migrations, or multi-step execution workflows) benefit from structured multi-agent coordination, iterative tool loops, and task decomposition.

Rather than embedding an expensive graph-compilation engine into the synchronous proxy routing hot path, Skail introduces the **Open-Multi-Agent (OMA)** Node.js sidecar (`@skail/oma-sidecar`). High-complexity requests are intercepted via **Proxy Complexity Gating** (`server/server_router.py`) and delegated asynchronously to the OMA runner (`skail/plugin/oma_sidecar/runner.js`).
...
1. **Top-Level Request**: `request_depth == 0` (extracted from `x-skail-depth` header).
...
The OMA sidecar is a pure Node.js (>=18) runner located at `skail/plugin/oma_sidecar/runner.js`.
...
When the OMA sidecar issues model completion requests back to Skail (`http://127.0.0.1:11434/v1`), it attaches:
```http
x-oma-sidecar: 1
x-skail-depth: 1
```
...
In accordance with Skail's core fail-soft invariant:
