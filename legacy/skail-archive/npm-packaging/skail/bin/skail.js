...
function findPython() {
  const candidates = process.env.SKAIL_PYTHON ? [process.env.SKAIL_PYTHON] : (process.platform === "win32" ? ["python3", "python", "py"] : ["python3", "python"]);
  return candidates[0];
}
const python = findPython();
if (!python) { console.error("[skail] Python 3.11+ is required. Set SKAIL_PYTHON."); process.exit(1); }
const env = { ...process.env };
const pkg = resolvePackage();
if (pkg) env.SKAIL_WHEEL_DIR = path.join(pkg, "python");
const child = spawn(python, [path.join(__dirname, "bootstrap.py"), ...process.argv.slice(2)], { stdio: "inherit", env });
child.on("error", (error) => { console.error(`[skail] unable to start Python: ${error.message}`); process.exit(1); });
