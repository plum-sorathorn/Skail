...
def package_dir(platform: str) -> Path:
    return ROOT / "npm-packaging" / f"skail-{platform}"
...
def wheel_path() -> Path:
    wheels = sorted((ROOT / "npm-packaging" / "dist").glob("skail-*-py3-none-any.whl"))
    if not wheels:
...
def build_one(platform: str, wheel: Path, check: bool = False) -> None:
    ...
    if check:
        ...
        for stale in (package_dir(platform) / "bin" / "skail", package_dir(platform) / "bin" / "skail.exe"):
