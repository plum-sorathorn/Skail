"""Real-process supervisor lifecycle tests for Skail launcher.
...
def _cleanup_supervisor(proc):
    child_pid = None
    child_path = proc._skail_state.parent / "run" / "child.pid" if hasattr(proc, "_skail_state") else None
...
p = pathlib.Path(os.environ['SKAIL_TEST_STATE'])
...
def test_supervisor_gives_up_after_rapid_failures(tmp_path):
    ...
    supervisor._skail_state = tmp_path / "starts"
    try:
