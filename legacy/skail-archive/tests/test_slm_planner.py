...
from skail.routing.slm_planner import (
    ExecutionPlan,
    SLMPlanner,
...
def mock_slm_generation():
    ...
    with patch("skail._compat.outlines_fallback.generate_structured_json", side_effect=mock_generate_structured_json):
        with patch("skail.routing.slm_planner.is_outlines_available", return_value=True):
            yield
...
async def test_strip_harness_noise_handles_sentinel_and_bracket_forms():
    ...
    from skail.routing.slm_planner import _strip_harness_noise
...
def test_resolve_slm_path_resolution(tmp_path, monkeypatch):
    ...
    from skail.routing.slm_planner import resolve_slm_path
    ...
    # 3. Path in ~/.skail/models/
    models_dir = tmp_path / "models"
    models_dir.mkdir()
    lfm_file = models_dir / "lfm2.5-1.2b-instruct-q4.onnx"
    lfm_file.write_bytes(b"LFM_DATA")

    with patch("skail.routing.slm_downloader.get_default_models_dir", return_value=models_dir):
...
def test_sanitize_task_type():
    ...
    """
    from skail.routing.slm_planner import sanitize_task_type, VALID_TASK_TYPES
...
def test_all_task_types_in_model_pool_weights():
    ...
    from skail.routing.model_pool import TASK_TYPE_WEIGHTS, capability_fit
    from skail.routing.slm_planner import VALID_TASK_TYPES
