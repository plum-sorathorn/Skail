...
from skail import auth
from skail.config import Config, resolve_api_key
from skail.providers import CustomEndpoint, generate_litellm_config
...
def test_in_memory_config_preservation_on_missing_file(tmp_path, monkeypatch):
    from skail.config.manager import get_config, _has_configured_models
...
def test_backup_recovery_on_cold_boot(tmp_path, monkeypatch):
    from skail.config.manager import load_config, _has_configured_models
    import skail.config.manager as manager_mod
...
def test_smart_fallback_provider_env_vars(monkeypatch):
    import skail.auth.auth as auth_mod
    from skail.config.resolver import resolve_orchestrator_model
