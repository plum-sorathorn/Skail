...
def test_server_alive_handles_failure():
    with patch("skail.launcher.urlopen", side_effect=OSError):
...
def test_path_block_is_idempotent_and_removable(tmp_path, monkeypatch):
    ...
    launcher.ensure_path_entry()
    assert bashrc.read_text().count("BEGIN SKAIL PATH") == 1
    launcher.remove_path_entry()
    assert "BEGIN SKAIL PATH" not in bashrc.read_text()


def test_windows_remove_path_entry_preserves_other_entries(monkeypatch):
    values = {"Path": (r"C:\Other;C:\Skail\bin;C:\Tools", 2)}
    writes = []
    registry = object()
    key = object()
    fake = type("WinReg", (), {
    ...
    monkeypatch.setattr(launcher, "shims_dir", lambda: Path(r"C:\Skail\bin"))
    launcher.remove_path_entry()
...
def test_claude_env_contains_router_settings():
    env = launcher._claude_env(11434, "skail")
    assert env["ANTHROPIC_BASE_URL"] == "http://127.0.0.1:11434"
    assert env["ANTHROPIC_AUTH_TOKEN"] == "skail-local"
    assert env["ANTHROPIC_MODEL"] == "skail"
...
def _supervisor_process(tmp_path, child_code):
    ...
    env = os.environ.copy()
    env["SKAIL_HOME"] = str(tmp_path)
    env["SKAIL_TEST_STATE"] = str(state)
