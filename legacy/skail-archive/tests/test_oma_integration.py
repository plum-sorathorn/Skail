...
async def test_high_complexity_triggers_oma(monkeypatch):
    ...
    monkeypatch.setattr("skail.plugin.runtime.start_task", mock_start_task)
...
async def test_recursion_header_bypasses_oma(monkeypatch):
    """x-skail-depth >= 1 or x-oma-sidecar=1 bypasses OMA gating to prevent infinite loops."""
    import skail.config.manager as m
    ...
    # Case 1: x-skail-depth: 1
    req1 = DummyRequest({"x-skail-depth": "1"})
...
async def test_fail_soft_when_oma_fails(monkeypatch):
    ...
    monkeypatch.setattr(
        "skail.plugin.runtime.start_task", mock_start_task_exc
    )
    ...
    monkeypatch.setattr(
        "skail.plugin.runtime.start_task", mock_start_task_err
    )
...
async def test_oma_never_returns_completed_result_without_runner_evidence(monkeypatch):
    ...
    m._config = cfg
    monkeypatch.setattr(
        "skail.plugin.runtime.start_task",
...
def test_oma_chat_completion_http_endpoint(monkeypatch):
    ...
    from skail.stats import aggregate, flush_stats, load_records
