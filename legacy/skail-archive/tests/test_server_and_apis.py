...
from skail import main, server_streaming
from skail.config import Config
from skail.jsonutil import parse_json_text
from skail.server.messages_api import (
    openai_messages_from_anthropic,
    openai_tool_choice_from_anthropic,
    openai_tools_from_anthropic,
    serve_model_ids,
)
from skail.server.messages_sse import AnthropicSSETranslator, anthropic_response_text, count_tokens
...
def test_anthropic_sse_translator_flow():
    translator = AnthropicSSETranslator("skail")
...
def test_models_endpoint(test_client):
    ...
    assert "skail" in ids
    assert "skail-budget" in ids
    assert "skail-expensive" in ids
...
def test_completions_with_orchestrator_tool_calls(test_client):
    ...
    cfg = get_config()
    dec = route([{"role": "user", "content": "hello"}], [], pseudo_model="skail", config=cfg)
...
async def test_openai_stream_records_one_stats_event_per_upstream_completion(monkeypatch):
    ...
    from skail.server import server_chat
    from skail.server.server_models import CompletionRequest
    ...
    response = await server_chat.handle_chat_completions(
        CompletionRequest(model="skail", messages=[{"role": "user", "content": "hi"}], stream=True),
        _ConnectedRequest(),
        PSEUDO_MODELS={"skail"},
    ...
    assert records[0][0][1:3] == ("skail", "openai/upstream")
...
async def test_openai_nonstream_records_route_metadata(monkeypatch):
    ...
    response = await server_chat.handle_chat_completions(
        CompletionRequest(model="skail", messages=[{"role": "user", "content": "hi"}]),
        _ConnectedRequest(),
        PSEUDO_MODELS={"skail"},
        route_target_fn=_route_to_upstream,
...
async def test_anthropic_completion_records_one_stats_event_per_upstream_completion(monkeypatch, stream):
    from skail import stats
    from skail.server import server_messages
    from skail.server.messages_api import (
    ...
    response = await server_messages.handle_messages(
        MessagesRequest(model="skail", messages=[{"role": "user", "content": "hi"}], max_tokens=10, stream=stream),
        _ConnectedRequest(),
        route_target_fn=_route_to_upstream,
