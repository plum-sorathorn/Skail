...
from skail import main as main_mod
...
def test_plugin_bias_e2e_flow(monkeypatch):
    ...
    try:
        ...
        import skail.config as _cfg_mod2

        monkeypatch.setattr(_cfg_mod2, "get_config", lambda: cfg)
    except Exception:
        pass
    try:
        import skail.server.server_streaming as _ss2
        # also ensure the already-imported config_module inside server_router sees patched cfg
        import skail.server.server_router as _sr2
        import skail.config as _cm2
    ...
    def _post_chat(
        ...
        if session_header is not None:
            headers["x-skail-session-id"] = session_header
        body = {"model": "skail", "messages": [{"role": "user", "content": "hello from e2e"}], "stream": stream}
        if payload_session_id is not None:
            headers["x-agent-id"] = "omp"
            body["skail_session_id"] = payload_session_id
    ...
    assert all("skail_session_id" not in kwargs for kwargs in dispatched_kwargs)

    # The private marker is also stripped from the streaming upstream path.
    _post_chat(payload_session_id=session, stream=True)
    assert "skail_session_id" not in dispatched_kwargs[-1]
