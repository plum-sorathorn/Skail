...
from skail.config.models import Config, ModelEntry, PluginConfig, SelectionConfig
...
def test_pseudo_model_variants_routing(eval_config: Config) -> None:
    ...
    dec_budget = route(prompt, config=eval_config, pseudo_model="skail-budget")
    assert dec_budget.model == "mock/gpt-4o-mini"

    dec_default = route(prompt, config=eval_config, pseudo_model="skail")
    assert dec_default.model == "mock/gpt-4o-mini"

    dec_expensive = route(prompt, config=eval_config, pseudo_model="skail-expensive")
...
async def test_server_startup_prewarms_slm() -> None:
    ...
    from skail.routing.slm_planner import _GLOBAL_LLM_CACHE
    ...
    async with lifespan(app):
        ...
        from skail.routing.slm_downloader import get_default_models_dir
