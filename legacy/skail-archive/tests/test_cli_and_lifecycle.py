...
from skail import main as cli
from skail import update
from skail.config import Config


def test_cli_version_flag(capsys):
    with patch.object(sys, "argv", ["skail", "--version"]):
        cli.main()
...
def test_cli_agent_shortcuts():
    with patch.object(cli, "cmd_launch_agent", return_value=0) as launch:
        with patch.object(sys, "argv", ["skail", "start", "--claude"]):
            try:
                cli.main()
    ...
    with patch.object(cli, "cmd_launch_agent", return_value=0) as launch:
        with patch.object(sys, "argv", ["skail", "start", "--pi"]):
            try:
                cli.main()
            except SystemExit as exc:
                assert exc.code == 0
    ...
    with patch.object(cli, "cmd_launch_agent", return_value=0) as launch:
        with patch.object(sys, "argv", ["skail", "start", "--opencode"]):
            try:
                cli.main()
            except SystemExit as exc:
                assert exc.code == 0
...
def test_cli_reset_command(capsys, monkeypatch, tmp_path):
    monkeypatch.setattr(cli, "home_dir", lambda: tmp_path / ".skail")
    args = MagicMock(force=True)
    with patch("skail.harnesses.all_adapters", return_value=[]):
        cli.cmd_reset(args)
    captured = capsys.readouterr().out
    assert "Coding agents reverted:" in captured
    assert "Skail state purged" in captured


def test_update_upgrade_commands():
    assert update.upgrade_command("uv-tool") == "uv tool upgrade --reinstall skail"
    assert update.upgrade_command("npm") == "npm install -g skail@latest"
    assert "pip install --force-reinstall --upgrade skail" in update.upgrade_command("pip")
...
def test_packaging_matrix():
    ...
    if build_path.exists():
        spec = importlib.util.spec_from_file_location("skail_packaging_build", build_path)
...
def test_run_proxy_suppresses_sigbreak_on_windows(monkeypatch):
    ...
    import sys
    from skail.server import server_streaming
