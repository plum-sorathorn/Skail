...
def isolate_home(tmp_path, monkeypatch):
    ...
    import skail.plugin.spool as spool_mod
...
def _run_hook(harness="claude", event="PostToolUse", stdin_data: str | None = None):
    ...
    from skail.cli.hook import cmd_hook
    import argparse
...
def test_hook_cli_wiring_skail_hook_command(tmp_path, monkeypatch):
    """Verify `skail hook claude <Event>` is registered in CLI parser."""
    from skail.cli.cli import main as cli_main
    # run via CLI parser: `skail hook claude PostToolUse` with empty stdin
    old_stdin = sys.stdin
    sys.stdin = io.StringIO("")
    try:
...
def test_hook_always_exit_0_even_on_spool_error(tmp_path, monkeypatch):
    ...
    import skail.cli.hook as hook_mod
...
async def test_spool_tailer_disabled_no_tailer(tmp_path, monkeypatch):
    ...
    from skail.plugin.spool import start_tailer, get_tailer, stop_tailer
...
async def test_spool_tailer_enabled_starts_and_counts(tmp_path):
    from skail.config.manager import get_config
    import skail.config.manager as m
    from skail.plugin.spool import start_tailer, stop_tailer, get_tailer
    from skail.plugin.ledger import get_ledger
...
def _patched_settings_path(tmp_path):
    # monkeypatch Path.home to tmp_path via fixture already set SKAIL_HOME,
...
def test_claude_patch_hooks_present_when_enabled(tmp_path, monkeypatch):
    ...
    # marker present
    assert data.get("skail", {}).get("hooks_enabled") is True
    assert data.get("skail", {}).get("ingestion_path") == "http"
...
def test_claude_patch_hooks_absent_when_disabled(tmp_path, monkeypatch):
    ...
    assert p.exists()
    data = json.loads(p.read_text(encoding="utf-8"))
    # no hooks key or no skail hook entries
...
def test_claude_patch_revert_removes_hooks(tmp_path, monkeypatch):
    ...
    adapter.revert()
    data = json.loads(p.read_text(encoding="utf-8"))
    # skail marker removed
    assert "skail" not in data
    # hooks either absent or without skail entries
    hooks = data.get("hooks")
    if hooks is not None:
        for entries in hooks.values():
            if isinstance(entries, list):
...
def test_claude_code_http_hooks_formatting_and_subagent_inclusion():
    ...
    for ev, entries in hooks.items():
        ...
        if "Subagent" in ev:
            assert h["_name"] == "Skail Subagent Tracker"
        else:
            assert h["_name"] == "Skail Monitor"

    mcp = adapter._build_mcp_block(port=12345)
    assert mcp == {"skail": {"url": "http://127.0.0.1:12345/mcp", "type": "http"}}
...
def test_claude_code_detects_and_strips_command_type_hooks():
    ...
    cmd_entry = {
        "matcher": "",
        "hooks": [{"type": "command", "command": "skail hook claude PreToolUse"}],
    }
    assert adapter._is_skail_hook_entry(cmd_entry) is True
    ...
    assert adapter._is_skail_hook_entry(http_entry) is True
    ...
    assert adapter._is_skail_hook_entry(user_entry) is False

    # Stripping hooks removes skail entries while preserving user hooks
    raw_hooks = {
        "PreToolUse": [cmd_entry, user_entry],
        "PostToolUse": [http_entry],
    }
    stripped = adapter._strip_skail_hooks(raw_hooks)
...
def test_pi_template_has_inert_gated_hook_section(tmp_path, monkeypatch):
    ...
    adapter = PiAdapter()
    adapter.patch(cfg)
    ext = tmp_path / ".pi" / "agent" / "extensions" / "skail.ts"
    assert ext.exists()
    text = ext.read_text(encoding="utf-8")
    assert "const SKAIL_HOOKS_ENABLED = false;" in text
    assert "const SKAIL_SUBAGENT_ENABLED = false;" in text
...
def test_pi_template_flag_true_when_pi_enabled(tmp_path, monkeypatch):
    ...
    ext = tmp_path / ".pi" / "agent" / "extensions" / "skail.ts"
    text = ext.read_text(encoding="utf-8")
    assert "const SKAIL_HOOKS_ENABLED = true;" in text
    assert "const SKAIL_SUBAGENT_ENABLED = true;" in text
...
def test_omp_extension_and_subagent_listeners(tmp_path, monkeypatch):
    ...
    assert "name: 'skail_search'" in rendered
    ...
    assert "const SKAIL_HOOKS_ENABLED = true;" in ext_content
    assert "const SKAIL_RAG_ENABLED = true;" in ext_content
...
def test_omp_extension_session_heartbeat_is_bounded_and_reports_metadata():
    ...
    assert "extension: 'skail'" in rendered
...
def test_omp_extension_marks_only_configured_provider_requests_with_session_id():
    ...
    assert "const SKAIL_PROVIDER = \"skail\";" in rendered
    assert 'const SKAIL_MODEL_IDS = new Set(["fast", "balanced", "frontier"]);' in rendered
    assert "pi.on('before_provider_request'" in rendered
    assert "currentModel?.provider !== SKAIL_PROVIDER" in rendered
    assert "SKAIL_MODEL_IDS.has(currentModel.id)" in rendered
    assert "SKAIL_MODEL_IDS.has(model)" in rendered
    assert "ctx?.sessionManager?.getSessionId?.()" in rendered
    assert "skail_session_id: sessionId.trim()" in rendered
...
def test_omp_session_activation_is_durable_and_visible_in_plugin_contract(tmp_path):
    ...
    from skail.plugin.ledger import get_ledger
    from skail.server.plugin_routes import install_plugin_routes
    import skail.config.manager as manager
...
def test_opencode_plugin_js_and_registration(tmp_path, monkeypatch):
    ...
    assert plugin_path == tmp_path / ".config" / "opencode" / "plugins" / "skail.js"

    rendered = adapter._render_plugin_js(port=11434, hooks_enabled=True, subagent_enabled=True)
    assert "SkailPlugin" in rendered
    ...
    assert "SkailPlugin" in plugin_path.read_text(encoding="utf-8")

    cfg_data = json.loads(opencode_cfg.read_text(encoding="utf-8"))
    assert "plugin" in cfg_data
    assert any("skail.js" in str(p) for p in cfg_data["plugin"])

    adapter.revert()
    assert not plugin_path.exists()
    cfg_data_reverted = json.loads(opencode_cfg.read_text(encoding="utf-8"))
    assert "plugin" not in cfg_data_reverted or not any("skail.js" in str(p) for p in cfg_data_reverted.get("plugin", []))
...
def test_opencode_stub_doc_exists():
    p = Path("skail/plugin/shims/README-opencode.md")
