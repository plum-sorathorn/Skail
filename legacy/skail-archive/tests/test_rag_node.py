...
try:
    ...
    from skail.knowledge.extractor import extract_repo_symbols
    from skail.knowledge.models import CodeChunk, QueryResult
except ImportError:
    # Also check compat layer if knowledge package is under construction
    try:
        from skail._compat.lancedb_fallback import connect as lancedb_connect
...
def test_rag_fallback_store_table_operations():
    ...
    from skail._compat.lancedb_fallback import LanceDBFallbackConnection
...
def test_rag_query_with_where_filter():
    ...
    from skail._compat.lancedb_fallback import connect

    db = connect(":memory:")
    table = db.create_table("contracts")
    table.add([
        {"name": "auth", "module": "skail.auth", "vector": [0.1, 0.1]},
        {"name": "server", "module": "skail.server", "vector": [0.2, 0.2]},
    ])
...
def test_rag_cosine_distance_computation():
    ...
    from skail._compat.lancedb_fallback import _cosine_distance
...
def test_mcp_routes_manifest_and_tool_calls(tmp_path):
    ...
    from skail.server.mcp_routes import install_mcp_routes, reset_mcp_singleton
    from skail.knowledge.vector_store import KnowledgeVectorStore
    ...
    res = client.post("/mcp/tools/call", json={"tool": "skail_index", "args": {"root_dir": str(tmp_path), "max_files": 10}})
    assert res.status_code == 200
    idx_data = res.json()
    ...
    res = client.post("/mcp/tools/call", json={"name": "skail_search", "arguments": {"query": "calculate_metrics"}})
    assert res.status_code == 200
    ...
    res = client.post("/mcp/tools/call", json={"tool": "skail_search", "args": {}})
    assert res.status_code == 200
    assert res.json().get("error") == "missing_query"

    res = client.post("/mcp/tools/call", json={"tool": "skail_index", "args": {}})
    assert res.status_code == 200
