...
from skail.tui.dashboard_widgets import move_cursor
...
try:
    ...
    def _require():
        raise RuntimeError("Textual is required to use the Skail TUI")
...
if _TEXTUAL:
    class UpdateScreen(Screen):
        ...
        async def _run_upgrade(self):
            ...
            from skail.config import load_config
            try:
                from skail.server import DEFAULT_PORT
            except ImportError:
                DEFAULT_PORT = 11434
            ...
            try:
                ...
                if proc.returncode == 0:
                    self._status = "[bold green][OK] Skail upgraded successfully! Please restart Skail.[/bold green]"
                else:
    ...
    class UpdateCatalogScreen(Screen):
        ...
        def compose(self):
            yield Vertical(
                Static("┌─ Skail · Refresh Model Data ─┐", markup=True),
        ...
        async def _sync(self):
            try:
                import asyncio
                from skail.routing.benchmarks import sync_openrouter_from_env
    ...
    class LaunchAgentScreen(Screen):
        ...
        def _load_agents(self):
            from skail.tui.onboarding import AGENTS, detect_agents, is_agent_configured
        ...
        def _launch(self):
            ...
            if not ag["configured"]:
                self._msg = f"Agent '{ag['id']}' is not configured — run: skail install {ag['id']}"
