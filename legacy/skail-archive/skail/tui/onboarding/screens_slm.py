...
from skail.routing.slm_downloader import (
    SLM_MODELS_CATALOG,
    download_slm_model,
    integrate_slm_model,
...
if _TEXTUAL:
    class SLMSetupScreen(Screen):
        ...
        def compose(self):
            ...
            yield Vertical(
                Static("┌─ Skail · Local SLM Engine Setup ─┐"),
                Static(
                    "Skail uses an embedded Small Language Model (SLM) for fast local task decomposition and dynamic orchestration.\n"
        ...
        async def _probe_health(self):
            try:
                ...
                from skail.config import get_config, normalize_api_base, provider_for, resolve_api_key
