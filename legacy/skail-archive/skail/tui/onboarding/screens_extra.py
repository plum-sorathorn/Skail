...
from skail.config import get_config, resolve_api_key, save_config
from skail.presets.model_presets import curated_model_catalog, resolve_models
...
class LauncherIntegrationScreen(Screen):
    ...
    def _install(self):
        try:
            self.query_one("#agents").update(
                "[bold cyan]Installing Skail agent integrations… please wait…[/bold cyan]"
            )
        except Exception:
            pass
