...
from skail.config import get_config, save_config, resolve_api_key
...
def render_slm_rows(models, selected_id, cursor, target_dir=None):
    from skail.routing.slm_downloader import is_slm_model_installed
    ...
    for p in adapter.config_paths():
        if p.exists():
            try:
                ...
                if "skail" in txt or "# begin skail" in txt:
                    return True
            except Exception:
                pass
    return False
...
def _persist(cfg):
    ...
    from skail.presets.model_presets import resolve_models
