...
class OmpAdapter(BaseAdapter):
    """Register Skail in OMP's YAML model registry."""
    ...
    def _extension_path(self) -> Path:
        return Path.home() / ".omp" / "agent" / "extensions" / "skail.ts"
    ...
    def _render_extension(
        ...
        if spool_path is None:
            try:
                ...
                spool_path = str(Path.home() / ".skail" / "run" / "plugin_spool.jsonl")
        ...
        return (
            f"// Skail Monitor & Router — managed by skail v{__version__}\n"
            "// Provides: provider routing, subagent tracking (agent_start/agent_end), codebase search\n"
            "// Reinstall: skail install omp\n"
            "// Remove:    skail uninstall omp\n"
            ...
            f"const SKAIL_EXTENSION_VERSION = {extension_version_json};\n"
            f"const SKAIL_PROVIDER = {provider_json};\n"
            f"const SKAIL_MODEL_IDS = new Set({pseudo_models_json});\n"
            "\n"
            ...
            "      if (\n"
            "        currentModel?.provider !== SKAIL_PROVIDER ||\n"
            "        !SKAIL_MODEL_IDS.has(currentModel.id) ||\n"
            "        typeof model !== 'string' ||\n"
            "        !SKAIL_MODEL_IDS.has(model)\n"
            "      ) return;\n"
            "      const sessionId = ctx?.sessionManager?.getSessionId?.();\n"
            "      if (typeof sessionId !== 'string' || !sessionId.trim()) return;\n"
            "      return { ...payload, skail_session_id: sessionId.trim() };\n"
            "    } catch {\n"
            "      return;\n"
            "    }\n"
            "  });\n"
            ...
            "            extension: 'skail',\n"
            "            extension_version: SKAIL_EXTENSION_VERSION,\n"
            "          },\n"
            "        });\n"
            "      }\n"
            "    });\n"
            ...
            "  }\n"
            "\n"
            "  if (SKAIL_RAG_ENABLED) {\n"
            "    const z = (pi as any).zod;\n"
            "    pi.registerTool({\n"
            "      name: 'skail_search',\n"
            "      label: 'Skail Search',\n"
            "      description: 'Search the Skail indexed knowledge base for codebase symbols.',\n"
            ...
            "            method: 'POST',\n"
            "            headers: { 'content-type': 'application/json' },\n"
            "            body: JSON.stringify({ tool: 'skail_search', args: params }),\n"
            ...
            "          }\n"
            "          return {\n"
            "            content: [{ type: 'text', text: 'Error: skail unavailable' }],\n"
            "            details: { error: 'skail unavailable' },\n"
            "            isError: true,\n"
            "          };\n"
            "        } catch {\n"
            "          return {\n"
    ...
    def patch(self, config: Config, port: int | None = None) -> None:
        ...
        providers[self.provider_name] = {
            ...
            "models": [
                {
                    "id": model,
                    "name": f"Skail {model.replace('-', ' ').title()}",
        ...
        model_roles["default"] = f"skail/{pseudo_model}"
