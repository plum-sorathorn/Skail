...
class ClaudeCodeAdapter(BaseAdapter):
    ...
    HOOK_COMMAND_TMPL = "skail hook claude {event}"
    ...
    def _build_hooks_block(self, port: int = 11434) -> dict:
        ...
        for ev in self.HOOK_EVENTS:
            label = "Skail Subagent Tracker" if "Subagent" in ev else "Skail Monitor"
            hooks[ev] = [
                {
                    "matcher": "",
                    "hooks": [
    ...
    def _build_mcp_block(self, port: int = 11434) -> dict:
        return {"skail": {"url": f"http://127.0.0.1:{port}/mcp", "type": "http"}}

    def _is_skail_hook_entry(self, entry: dict) -> bool:
        try:
            ...
            for h in inner:
                if isinstance(h, dict):
                    ...
                    if "skail hook claude" in cmd or "/plugin/events" in url or name.startswith("Skail"):
                        return True
            return False
        except Exception:
            return False
    ...
    def _merge_hooks(self, existing: dict | None, port: int = 11434) -> dict:
        """Merge skail hook entries into existing hooks dict, idempotent."""
        base: dict[str, list] = {}
        if isinstance(existing, dict):
            # copy user hooks, but strip old skail entries to avoid duplicates
            for k, v in existing.items():
                if not isinstance(v, list):
                    base[k] = v  # type: ignore[assignment]
                    continue
                kept = [e for e in v if not self._is_skail_hook_entry(e)]
                if kept:
                    base[k] = kept
    ...
    def _strip_skail_hooks(self, existing: dict | None) -> dict | None:
    ...
    def patch(self, config: Config, port: int | None = None) -> None:
        ...
        if not isinstance(data, dict):
            data = {}
        first_takeover = not isinstance(data.get("skail"), dict)
        ...
        values = _claude_env(
            effective_port, getattr(config, "pseudo_model", "skail")
        )
        marker = (
            data.get("skail") if isinstance(data.get("skail"), dict) else {}
        )
        ...
        if hooks_enabled:
            ...
            # flags off → ensure no skail hooks remain (byte-identical to pre-plugin when never enabled)
            stripped = self._strip_skail_hooks(existing_hooks if isinstance(existing_hooks, dict) else None)
            if stripped is None:
                data.pop("hooks", None)
            else:
                ...
                if isinstance(marker, dict):
                    ...
                    if "previous_hooks" in marker:
                        if prev_hooks is None:
                            ...
                            stripped = self._strip_skail_hooks(data.get("hooks") if isinstance(data.get("hooks"), dict) else None)
                            if stripped is None:
                                data.pop("hooks", None)
                            elif not stripped:
                                data.pop("hooks", None)
