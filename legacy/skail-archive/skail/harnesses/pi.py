...
class PiAdapter(BaseAdapter):
    """Register Skail as a custom provider for Pi.
    ...
    #: the default.
    PSEUDO_MODELS = ("skail", "skail-budget", "skail-expensive")
    ...
    def _extension_path(self) -> Path:
        return self._agent_dir() / "extensions" / "skail.ts"
    ...
    def _render_extension(
        ...
        return (
            ...
            "// Reinstall: skail install pi\n"
            "// Remove:    skail uninstall pi\n"
    ...
    def patch(self, config: Config, port: int | None = None) -> None:
        ...
        pseudo_model = str(
            getattr(pi_settings, "model", None) or getattr(config, "pseudo_model", None) or "skail"
        )
