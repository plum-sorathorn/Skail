...
def cmd_launch_agent(agent_id: str, port: int | None = None, new_terminal: bool | None = None) -> int:
    """Launch an agent by ID through the Skail proxy server.
    ...
    cmd = [python_bin, "-m", "skail", "start", "--headless", "--port", str(port)]

    if use_new_terminal:
        print(f"Skail: launching proxy in a new terminal window (port {port})…")
    ...
    try:
        ready_budget = max(
            30.0, float(os.environ.get("SKAIL_READY_TIMEOUT", "90.0"))
        )
    except ValueError:
        ready_budget = 90.0
    ...
    if not real_bin:
        print(
            f"agent '{agent_id}' not found on PATH; run: skail install {agent_id}"
        )
    ...
    if agent_id == "claude_code":
        ...
        env["OPENAI_API_KEY"] = "skail-local"
        env["OPENCODE_MODEL"] = f"skail/{pseudo}"
    elif agent_id == "pi":
        env["AI_AGENT"] = "pi"
    ...
    print(
        f"Skail ready at http://127.0.0.1:{port} — launching {adapter.binary_name}"
    )
    try:
