...
def _claude_env(port: int, pseudo: str = "skail") -> dict[str, str]:
    return {
        "ANTHROPIC_BASE_URL": f"http://127.0.0.1:{port}",
        "ANTHROPIC_AUTH_TOKEN": "skail-local",
        ...
        "ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION": "Skail local router",
...
def _claude_env_blocks(port: int, pseudo: str = "skail") -> tuple[str, str]:
...
def _omp_env_blocks(port: int, pseudo: str = "skail") -> tuple[str, str]:
    """Return OMP's proxy endpoint and default model environment."""
    return (
        'export OMP_BASE_URL="http://127.0.0.1:${PORT}/v1"\n'
        'export OMP_API_KEY="skail-local"\n'
        f'export OMP_MODEL="skail/{pseudo}"',
        'set "OMP_BASE_URL=http://127.0.0.1:%PORT%/v1"\n'
        'set "OMP_API_KEY=skail-local"\n'
        f'set "OMP_MODEL=skail/{pseudo}"',
    )


def _opencode_env_blocks(port: int, pseudo: str = "skail") -> tuple[str, str]:
    """Return environment variables for OpenCode."""
    bash = (
        'export OPENAI_BASE_URL="http://127.0.0.1:${PORT}/v1"\n'
        'export OPENAI_API_KEY="skail-local"\n'
        f'export OPENCODE_MODEL="skail/{pseudo}"'
    )
    cmd = (
        'set "OPENAI_BASE_URL=http://127.0.0.1:%PORT%/v1"\n'
        'set "OPENAI_API_KEY=skail-local"\n'
        f'set "OPENCODE_MODEL=skail/{pseudo}"'
    )
    return bash, cmd
...
def shim_script(agent_id, real_bin):
    ...
    pseudo = getattr(cfg, "pseudo_model", "skail") or "skail"
    lines = [
        "#!/usr/bin/env bash",
        "set -u",
        f"REAL_BIN={shlex.quote(real_bin)}",
        f"PY={shlex.quote(sys.executable)}",
        f'PORT="${{SKAIL_PORT:-{cfg.port}}}"',
    ]
    ...
    lines.append('"$PY" -m skail ensure --port "$PORT" --client-id "$SHIM_ID" || true')
    lines.append('"$REAL_BIN" "$@"')
    lines.append("rc=$?")
    lines.append('"$PY" -m skail release --port "$PORT" --client-id "$SHIM_ID" || true')
...
def shim_script_win(agent_id, real_bin):
    ...
    lines.append('"%PY%" -m skail ensure --port %PORT% --client-id %SHIM_ID%')
    lines.append('"%REAL_BIN%" %*')
    lines.append("set RC=%ERRORLEVEL%")
    lines.append('"%PY%" -m skail release --port %PORT% --client-id %SHIM_ID%')
...
def ensure_path_entry():
    ...
    changed = None
    block = '# BEGIN SKAIL PATH\nexport PATH="$HOME/.skail/bin:$PATH"\n# END SKAIL PATH\n'
    for rc in (Path.home() / ".bashrc", Path.home() / ".zshrc"):
        if rc == Path.home() / ".zshrc" and not rc.exists():
            continue
        text = rc.read_text() if rc.exists() else ""
        if "# BEGIN SKAIL PATH" not in text:
            rc.write_text(
