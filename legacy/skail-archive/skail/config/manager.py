...
from skail.config.paths import backups_dir, config_path, run_dir
...
def load_config(
    ...
    if "SKAIL_PORT" in os.environ:
        data["port"] = int(os.environ["SKAIL_PORT"])
    if "SKAIL_LOG_LEVEL" in os.environ:
        data["log_level"] = os.environ["SKAIL_LOG_LEVEL"]
    ...
    if snapshot.exists():
        try:
            from skail.routing.benchmarks import load_snapshot
            load_snapshot(snapshot, max_age_hours=config.selection.benchmark_snapshot_max_age_hours)
        except Exception as exc:
            logging.getLogger("skail").warning("Ignoring invalid benchmark snapshot %s: %s", snapshot, exc)
            from skail.routing.benchmarks import clear_registry
            clear_registry()
    else:
        from skail.routing.benchmarks import clear_registry
        clear_registry()
    ...
    if any(
        ...
        from skail.auth.auth import migrate_from_config
    ...
    with _config_lock:
        ...
        if not saved:
            try:
                ...
                logging.getLogger("skail").error("Failed to save config to %s: %s", p, exc)
            finally:
                if temp_path.exists():
                    try:
                        temp_path.unlink()
