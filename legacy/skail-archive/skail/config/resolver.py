...
from skail.config.models import ModelEntry
...
def resolve_api_key(entry: dict[str, Any], provider: str | None = None) -> str:
    ...
    try:
        from skail.auth.auth import get_provider_key
...
def resolve_orchestrator_model(cfg: Any = None) -> str:
    ...
    for entry in _configured_model_sources(cfg):
        ...
        if model:
            return str(model)
    try:
        from skail.auth.auth import load_auth
...
def select_model_by_tier(tier: str, cfg: Any = None) -> str:
    ...
    try:
        from skail.routing.pricing import select_for_sla
...
def orchestrator_litellm_params(cfg: Any = None) -> dict[str, str]:
    ...
    from skail.server.messages_api import litellm_params_for
    from skail.config.manager import get_config
