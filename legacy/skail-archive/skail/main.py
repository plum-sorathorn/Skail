...
from skail import cli as _cli
from skail import server as _server

try:
    from skail.server import app as _app  # noqa: F401
    app = _app
except Exception:
    app = None  # type: ignore[assignment]

from skail.cli import (  # noqa: F401
...
import skail.cli.cli as _cli_sub
...
from skail.server import (  # noqa: F401
    DEFAULT_PORT,
...
def _run_supervisor_impl(port, log_level="info", host="127.0.0.1", child_cmd=None):  # type: ignore[no-untyped-def]
    ...
    # patching skail.main.* affect the supervisor loop.
    for _extra in ("time", "subprocess", "home_dir"):
        if _extra in globals():
            try:
                ...
                pass
            try:
                import skail.server.server_streaming as _ss
...
# Export as _run_supervisor (tests patch skail.main._run_supervisor)
