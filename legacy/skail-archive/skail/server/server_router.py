...
def is_active_tool_session(messages: list[Any]) -> bool:
    ...
    is managing its own tool execution loop. Skail relays requests
...
async def call_litellm(
    ...
    *,
    ...
    from skail.server.server_streaming import _litellm

    llm = _litellm()
    if llm is None:
        raise RuntimeError("litellm unavailable")
    kwargs = body.model_dump(exclude_none=True)
    kwargs.pop("skail_session_id", None)
...
async def route_target(
    ...
    *,
    ...
    try:
        from skail.server.messages_models import normalize_pseudo_model
    ...
    if request is not None and hasattr(request, "headers"):
        try:
            request_depth = int(request.headers.get("x-skail-depth", "0"))
    ...
    if session_id is None and request is not None and hasattr(request, "headers"):
        try:
            header_session_id = (
                request.headers.get("x-skail-session-id")
    ...
    if body_model in PSEUDO_MODELS:
        ...
        if on_progress is not None:
            try:
                ...
                pass
        try:
            from skail.stats import update_active_routing
        ...
        if not model:
            from skail.config import resolve_orchestrator_model
    ...
    if request_depth >= 1:
        ...
        if not is_high_complexity:
            ...
            try:
                from skail.plugin import runtime
