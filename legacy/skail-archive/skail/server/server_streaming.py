...
from skail.config import get_config, home_dir
...
def _write_crash_report(exc):
    ...
    with crash_path.open("a", encoding="utf-8") as stream:
        stream.write(f"\n--- Skail crash at {time.strftime('%Y-%m-%d %H:%M:%S')} ---\n")
...
def _build():
    ...
    from skail.stats import install_recorder
    ...
    try:
        ...
        async def _plugin_lifespan(app_inner):
            ...
            try:
                from skail.plugin.spool import start_tailer
            ...
            # shutdown
            try:
                from skail.plugin.spool import stop_tailer
                from skail.plugin.ledger import get_ledger as _get_ledger2
                try:
                    await stop_tailer()
                except Exception:
                    pass
...
def _run_proxy(port: int, log_level: str = "info", host: str = "127.0.0.1"):
    ...
    configured_level = os.environ.get("SKAIL_LOG_LEVEL", "INFO").upper()
    ...
    try:
        ...
        async def _patched_startup(sockets=None):
            ...
            _write_ready()
            logging.getLogger("skail").info(
                "Skail proxy ready at http://%s:%d (Press CTRL+C to quit)", host, port
            )
        ...
        logger.exception("Skail proxy crashed")
...
def _run_supervisor(
    ...
    """
    from skail.launcher import _create_kill_on_close_job, daemon_python
    ...
    try:
        while not stopping:
            ...
            if len(failures) >= SUPERVISOR_MAX_RAPID_FAILURES:
                ...
                logger.error(
                    "supervisor giving up after %d failures in %.0fs — run 'skail start' to relaunch",
                    SUPERVISOR_MAX_RAPID_FAILURES,
...
def _check_port_available(port: int, host: str = "127.0.0.1") -> None:
    ...
    if os.environ.get("SKAIL_SUPERVISED") == "1":
