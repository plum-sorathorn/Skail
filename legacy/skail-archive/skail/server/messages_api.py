...
from skail.config import normalize_api_base, qualify_model, resolve_api_key
from skail.presets import normalize_model_id_for_provider
...
def custom_entry(cfg, model_id: str) -> dict | None:
    from skail.config import _configured_model_sources
...
def litellm_params_for(model_id: str, cfg) -> dict:
    from skail.config import provider_for
...
def messages_litellm_kwargs(model_id: str, extra: dict | None = None) -> dict:
    from skail.config import qualify_model
