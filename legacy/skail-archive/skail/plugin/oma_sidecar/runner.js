...
async function runFallback(payload, selectedMode, workspaceTools) {
  ...
  const model = payload.model || 'skail';
  ...
  try {
    if (typeof fetch === 'function') {
      ...
      const sysMsg = `You are an Open-Multi-Agent (OMA) runner operating in ${selectedMode} mode for Skail. ` +
  ...
  const report = llmContent ||
    ...
    `Result: Task completed successfully via Skail fallback runner.`;
