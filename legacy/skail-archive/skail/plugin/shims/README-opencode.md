...
Status: **document-only** this phase. No code changes to `skail/harnesses/opencode.py`
...
- The shim will register a single hook plugin entry:
  ...
  {
    "plugin": "skail-hook",
    "entry": "~/.skail/run/opencode-hook.mjs",
    "config": { "endpoint": "plugin_spool.jsonl" }
  }
  ```

  The plugin itself appends one JSONL line to `~/.skail/run/plugin_spool.jsonl`
...
- `skail install opencode` with `plugins.enabled && plugins.opencode_enabled`
  injects the plugin block inside `# BEGIN/END SKAIL` markers; `revert()` removes it.
