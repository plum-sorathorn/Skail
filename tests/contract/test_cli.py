...
def run_module(*arguments: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "rudder.cli.main", *arguments],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
...
def test_help_identifies_the_native_rudder_command() -> None:
    result = run_module("--help")
    assert result.returncode == 0
    assert "Rudder" in result.stdout
    assert "AutoConduck" not in result.stdout


def test_version_is_the_new_distribution_version() -> None:
    result = run_module("--version")
    assert result.returncode == 0
    assert result.stdout.strip() == "rudder 0.1.0"


def test_fake_provider_smoke_is_offline_and_deterministic() -> None:
    result = run_module("smoke", "--fake-provider")
    assert result.returncode == 0
    assert result.stdout.strip() == "Rudder fake-provider smoke: ok"
...
def test_installed_console_script_exposes_help_and_version() -> None:
    ...
    assert "Rudder" in help_result.stdout
    assert version_result.returncode == 0
    assert version_result.stdout.strip() == "rudder 0.1.0"
