...
def test_r1_estimate_cost_nonzero_and_record_writes_jsonl(tmp_path, monkeypatch):
    ...
    from autoconduck import stats as stats_mod
    from autoconduck.config import Config
    from autoconduck.config.paths import config_path
    ...
    with patch("autoconduck.config.get_config", return_value=cfg):
    ...
    stats_mod.record("FAST", "autoconduck", "gpt-4o-mini", 100, 50, success=True)
...
def test_r1_record_does_not_raise_on_missing_pricing_record_usage(monkeypatch, tmp_path):
    ...
    import autoconduck.routing.pricing as pricing_mod
    ...
    try:
        ...
        # Should not raise
        stats_mod.record("FAST", "autoconduck", "some-model", 10, 20)
        stats_mod.flush_stats()
        jf = fake_home / "run" / "stats.jsonl"
        assert jf.exists()
...
def test_r4_tier_derivation():
    from autoconduck.routing.dispatcher import _tier_from_pseudo, _select_planned
    from autoconduck.routing.model_pool import CapabilitySLA
    from autoconduck.config import Config
    assert _tier_from_pseudo("autoconduck-budget") == "budget"
    assert _tier_from_pseudo("autoconduck-expensive") == "expensive"
    assert _tier_from_pseudo("autoconduck") == "default"
    ...
    tier = _tier_from_pseudo("autoconduck-budget")
    info = _select_planned(sla, FakePlan(), cfg, "autoconduck-budget", tier, None)
    assert info.model == "cheap-model", f"budget cap should select cheap, got {info.model}"
    # default tier allows mid
    tier2 = _tier_from_pseudo("autoconduck")
    info2 = _select_planned(sla, FakePlan(), cfg, "autoconduck", tier2, None)
    ...
    tier3 = _tier_from_pseudo("autoconduck-expensive")
    info3 = _select_planned(sla, FakePlan(), cfg, "autoconduck-expensive", tier3, None)
...
def test_r5_plan_sync_circuit_breaker_timeout_returns_fallback(monkeypatch):
    ...
    def slow_infer(self, messages, config=None):
        time.sleep(0.5)
        return {"confidence": 0.9, "task_type": "chat", "complexity_score": 2, "suggested_sla": __import__("autoconduck.routing.model_pool", fromlist=["CapabilitySLA"]).CapabilitySLA(), "rationale": "slow"}
...
def test_r5_plan_sync_success_still_works(monkeypatch):
    ...
    def fast_infer(self, messages, config=None):
        return {"confidence": 0.8, "task_type": "chat", "complexity_score": 2, "suggested_sla": __import__("autoconduck.routing.model_pool", fromlist=["CapabilitySLA"]).CapabilitySLA(), "rationale": "fast"}
...
def test_r6_bias_ttl_not_incremented_by_plugin_events():
    ...
    from autoconduck.server.plugin_routes import _events_handler
    import inspect
    ...
    import autoconduck.server.server_router as sr_mod
...
async def test_r7_route_target_failure_never_500(monkeypatch):
    from autoconduck.server.server_messages import handle_messages
