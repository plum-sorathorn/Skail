# ADR 0001: Build Skail as a Native Multi-Agent Harness
...
1. Skail will be a native terminal coding harness, not a proxy, plugin, daemon, or compatibility layer for another harness.
...
3. Superseded in part by ADR 0006. Skail retains DeepAgents' standard `task` surface, but routes it
   through the same Skail-owned adaptive plan admission and lifecycle services as typed plans.
...
- A compiled lifecycle wrapper gives Skail deterministic ownership of the policies that matter economically and operationally.
...
- Framework adapters prevent a preview feature or dependency upgrade from defining Skail's public contracts.

## Alternatives considered

### Rebrand the existing proxy/plugin system

Rejected because Skail would still lack authoritative control of the lead loop, tools, child scheduling, and task state. It would retain the integration complexity the new product is meant to remove.

### Build a harness from scratch

Rejected because the agent loop, graph state, checkpointing, tool middleware, subagents, and interrupts are already available in DeepAgents/LangGraph. Skail's differentiator is orchestration and economics, not reimplementing those foundations.

### Fork or clone dcode

Rejected as the architectural foundation. Skail should emulate the proven behavior of a capable lead with selective delegation, but use a maintained framework boundary and its own policies rather than depend on another product's internal implementation.
...
- Skail has one coherent runtime and user experience.
...
- Skail complexity can be removed rather than kept as compatibility debt.
