...
Status: Accepted
Date: 2026-09-02
Depends on: [ADR 0001](./0001-skail-native-multi-agent-harness.md)

## Context

Skail depends on model assignment occurring before a compiled child's first call, stable nested
streaming, checkpointed interrupts, concurrent synchronous subagents, and cooperative
cancellation. DeepAgents is a beta dependency and these framework behaviors are not Skail-owned
...
Skail will consume these packages behind `skail.runtime` adapters and retain contract tests for
...
1. The standard `task` tool exposes only `description` and `subagent_type`; Skail lifecycle state
...
4. Tool-indirected child state cannot be reconstructed from parent checkpoint inspection alone, so
   Skail's journal remains authoritative for task visibility and reconciliation.

DeepAgents currently brings Anthropic and Google integrations transitively. Skail will not expose
or initialize them without explicit provider configuration or credentials. Provider support and
automatic-routing eligibility remain Skail-owned, lazy, evidence-bearing decisions.
