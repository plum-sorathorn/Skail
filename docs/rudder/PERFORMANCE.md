...
Heavy framework imports (`deepagents`, `langchain`, `langgraph`) are deferred until a task run instruction actually begins. CLI argument parsing, subcommand help (`rudder --help`), session lists (`rudder sessions list`), and configuration inspection are measured independently from first useful runtime response.
