...
Rudder should use, rather than reproduce:
...
- compiled subagents through `CompiledSubAgent` for Rudder-owned task lifecycle graphs;
...
DeepAgents subagents are not merely static nodes in the lead graph. The lead calls a `task` tool; the subagent middleware invokes a separately compiled runnable. Rudder makes that runnable a small task lifecycle graph whose execution node is itself a DeepAgent. This gives Rudder an enforceable assignment and retry boundary while retaining the framework's context isolation and nested streaming.

Framework assumptions must be covered by contract tests because they are not Rudder-controlled APIs. The source baseline used for this design is listed in [README.md](./README.md).

## 4. What Rudder owns

Rudder owns these semantics and may not delegate them to prompts:
...
- a system prompt assembled from Rudder rules, user instructions, trusted project instructions, and the selected lead profile;
- default tools assembled by `ToolRegistry`;
- DeepAgents filesystem, execution, todo, skills, memory, and subagent middleware;
- a standard `task` tool whose built-in profiles are supplied as compiled Rudder task graphs;
...
- Rudder's scheduler/semaphore still enforces the configured maximum;
...
The lead supplies `TaskRequest`. Rudder creates `TaskSpec`; model-generated IDs, budgets, or permission claims are ignored.
...
`max_agents` is a hard semaphore with allowed values 1–3. A process-local semaphore is sufficient for the first release because one interactive Rudder process owns a session. Persistence still records lease ownership so interrupted sessions can recover safely.
...
2. create a task-specific branch/worktree under Rudder's data directory;
...
Both are first-class OpenAI-compatible provider configurations because they are important AutoConduck capabilities. LLM Gateway's canonical API base is `https://api.llmgateway.io/v1`. Contract tests must cover:
...
OpenAI compatibility must not imply unsupported provider-specific response fields are preserved. Rudder exposes only normalized capabilities proven by tests.

### 11.3 Provider breadth

Rudder accepts any LangChain-supported provider for which a model factory can be constructed. The core install stays small; provider integrations use extras or user-installed packages. “Supported” has precise levels:
...
DeepAgents filesystem permissions do not secure `execute`, custom tools, or MCP tools. Rudder wraps each through `ExecutionPolicy`:
...
- `~/.rudder/checkpoints.sqlite`: LangGraph checkpointer state;
- `~/.rudder/rudder.sqlite`: Rudder-owned session, task, route, usage, approval, and event journal;
- `~/.rudder/config.toml`: user configuration;
- `~/.rudder/agents/` and `~/.rudder/skills/`: user extensions;
- `~/.rudder/cache/`: model catalog and non-authoritative caches;
- `~/.rudder/logs/`: scrubbed diagnostic logs under retention policy.
...
3. reconcile Rudder tasks using idempotency keys;
