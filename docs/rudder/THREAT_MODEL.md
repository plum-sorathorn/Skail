...
Rudder executes locally as a developer pairing tool without cloud daemons, proxy servers, or plugin planes. Trust boundaries are enforced in deterministic application code, never via model prompt compliance.
...
- **Sensitive Path Guard**: Common credential patterns (`.env`, `id_rsa`, `*.pem`, `*.key`, `.git`, `.rudder`) cannot be read into agent context without explicit elevation.
...
- **Threat**: Repository contains malicious `.rudder/agents/` or `.rudder/skills/` attempting privilege escalation.
