...
Let the user start productive work with `rudder` and a prompt, without starting a proxy, selecting an orchestration graph, or attaching Rudder to another harness.

### Trigger and flow

1. `rudder` resolves configuration, workspace, project trust, and session choice.
...
- A second Rudder process may read other sessions but must not concurrently mutate a session owned by a live process.

## 2. Capable lead agent

### Purpose

Keep Rudder useful for simple work and resilient when delegation is unnecessary or unsuccessful.
...
- explicit “use N agents” is honored up to configured and hard limits, or Rudder explains the conflict.
...
Rudder rejects or blocks a request when:
...
1. Trusted project `.rudder/agents/<name>/AGENTS.md`.
2. User `~/.rudder/agents/<name>/AGENTS.md`.
...
A model is automatically eligible only if its compatibility and capability evidence is trusted. Unknown models remain manually usable. Rudder must never infer quality from model name, vendor reputation, price, or parameter-count text.
...
- maintained Rudder catalog entries;
- provider model discovery;
- trusted user overrides;
- replayable Rudder evaluations;
- provider pricing/capability metadata where authoritative.

### Merge order

User overrides > evaluated Rudder evidence > maintained entries > discovered metadata. A higher source may fill or explicitly replace fields, and provenance is retained per field.
...
Early provider failures influence health and fallback, not global capability. Rudder does not learn a model-quality score from a handful of private sessions in the first release.
...
LLM Gateway is first-class. Rudder uses its OpenAI-compatible `/v1` endpoint, model discovery when available, and normalizes streaming tool calls and usage through a dedicated adapter. The adapter is contract-tested instead of assuming every OpenAI extension behaves identically.
...
Before a child batch starts, Rudder retains enough budget for at least one lead synthesis call on the assigned lead model. If this cannot be retained, fan-out is reduced or blocked. Rudder must not spend the full budget on children and strand the user without a final answer.
...
The stronger model continues from the existing workspace state. Rudder does not automatically erase partial work.

### Second failed attempt

Rudder marks `returned_to_lead` and supplies both summaries. The lead may repair directly, materially re-scope, or ask the user. The same task cannot be automatically submitted for attempt three.
...
Rudder adopts useful DeepAgents tools: list, glob, grep, read, write, edit, execute, todos, subagents, skills, and memory, plus Rudder's user-question tool.
...
On first use of a canonical workspace containing Rudder project configuration/extensions, the TUI summarizes requested capabilities before asking for trust. Merely opening the workspace does not execute project code.
...
Rudder memory means deliberate user/project instructions and bounded session summaries, not silent semantic harvesting. The first release uses file-backed DeepAgents-compatible memory. The user can inspect and edit it.
...
Resume restores transcript, lead/task graph states, assignments, task tree, usage, reservations, pending questions, and trust context. Orphaned in-flight provider calls become interrupted; Rudder does not charge or replay them without reconciliation evidence.
...
| AutoConduck feature | Rudder decision | Reason / replacement |
|---|---|---|
| OpenAI-compatible proxy and HTTP server | Remove | Rudder invokes providers directly as a harness. |
| Router-only and plugin operating modes | Remove | One native runtime owns agent and tool execution. |
| Plugin plane and `/plugin/*` endpoints | Remove | Task lifecycle is in Rudder's graph/runtime. |
| OMA Node sidecar and complexity intercept | Remove | DeepAgents subagents plus Rudder task graphs provide delegation. |
...
| Harness shims/hooks/spool files | Remove | Rudder is the harness, not an observer of Claude Code/Pi/OpenCode. |
| Agent config mutation/backups | Remove | Rudder reads its own config and never patches another harness. |
...
12. Legacy AutoConduck code is reference-only and cannot be imported by Rudder.
