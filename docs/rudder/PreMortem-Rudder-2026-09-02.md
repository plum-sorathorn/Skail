# Pre-Mortem Analysis: Rudder

Status: Proposed risk analysis
Date: 2026-09-02
Scenario: Rudder launches in 14 days, on 2026-09-16, and fails. This is a stress-test date, not an implementation schedule.

## Failure narrative

Rudder launched with an appealing “just prompt, we'll orchestrate” story, but early users found that multi-agent runs cost more than a strong single model, parallel writers interfered, and model selection was hard to trust. Provider-specific tool calling broke despite simple chat tests. Users could not distinguish running, queued, approval-blocked, and retrying work. A few interrupted sessions resumed with stale reservations or repeated work. The team spent launch week repairing framework and provider behavior instead of proving that Rudder improved completed-work economics.
...
**Why real:** Rudder depends on assigning a model before a child starts and keeping it stable. Child state propagation, middleware order, nested streaming, and cancellation are framework contracts; async subagents are preview.
...
**Urgency:** Launch-blocking
**Why real:** LangGraph checkpoints and Rudder's journal cannot share one transaction. A crash between provider execution, settlement, terminal state, and checkpointing can create conflicting truths.
...
### T8. The rewrite recreates AutoConduck complexity under new names
...
### P1. Rudder must use as few tools as Pi

Pi's restraint is useful, but an exact tool count is not the goal. Rudder needs files, execution, tasks, todos, skills, memory, and questions. Duplicate tools, unclear side effects, and excessive outputs are the actual risks.

### P2. Removing OMA and the SLM removes all intelligence

A capable lead can delegate through DeepAgents, while Rudder enforces lifecycle, budgets, routing, concurrency, and failure policy. Rebuilding OMA would add a second planner without proven value.
...
### P4. No AutoConduck migration helper loses every existing user

Rudder is intentionally a new product with incompatible concepts. Leaving old data untouched and documenting fresh setup is sufficient; a compatibility layer would preserve removed architecture.
